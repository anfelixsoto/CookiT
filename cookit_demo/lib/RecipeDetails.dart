...
class RecipeDetails extends StatefulWidget {
  ...
  final Recipe recid;
  RecipeDetails({Key key, @required this.recipe, this.recipeId, this.recid})
      : super(key: key);
...
class _RecipeDetails extends State<RecipeDetails> {
  ...
  Future<List<String>> getFavorites() async {
    ...
    if (querySnapshot.exists &&
      ...
      setState(() {
        if (widget.recipeId != null &&
            List<String>.from(querySnapshot.data['favorites'])
                .contains(widget.recipeId.rid.toString())) {
          favorite = true;
        } else if (widget.recid != null &&
            List<String>.from(querySnapshot.data['favorites'])
                .contains(widget.recid.id.toString())) {
          favorite = true;
  ...
  Future<List<String>> getSaved() async {
    ...
    if (querySnapshot.exists &&
      ...
      setState(() {
        if (widget.recipeId != null &&
            List<String>.from(querySnapshot.data['saved'])
                .contains(widget.recipeId.rid.toString())) {
          saved = true;
        } else if (widget.recid != null &&
            List<String>.from(querySnapshot.data['saved'])
  ...
  Future<void> getUserRef() async {
    ...
    setState(() {
      ...
      userRef.get().then((data) {
        ...
        setState(() {
          if (widget.recipeId != null &&
              List<String>.from(data.data["favorites"])
                  .contains(widget.recipeId.rid.toString())) {
            favorite = true;
          } else if (widget.recid != null &&
              List<String>.from(data.data["favorites"])
                            ...
                            children: <Widget>[
                              ...
                              Container(
                                ...
                                child: ListView(
                                  padding: EdgeInsets.symmetric(horizontal: 10),
                                  children: <Widget>[
                                    new Text(recipe.numCalories.toString(),
                                        textAlign: TextAlign.center,
                                        style: TextStyle(
                                            fontWeight: FontWeight.bold,
                                            fontSize: 20.0,
                                          ...
                                          MaterialPageRoute(
                                            builder: (context) =>
                                                RecipeInstructions(
                                                    recipe: postRecipe,
                                                    rId: postRecipe,
                                                    dbId: widget.recipeId),
                                          ));
                                    });
                                  } else {
                                    Firestore.instance
                                        .collection('recipes')
                                        .document(widget.recid.id.toString())
                                        .get()
                                        .then((data) {
                                      //print(data.documentID);
                                      Recipe postRecipe = Recipe.fromDoc(data);
    ...
    if (favorite == false) {
      return FlatButton(
        ...
        onPressed: () {
          ...
          if (widget.recipeId != null) {
            UserOperations.addToFavorites(
                userId, widget.recipeId.rid.toString());
            RecipeOperations.addToRecipes(
                widget.recipeId.rid.toString(), recipe);
          } else {
